...
owner_approval: Phase 1 received (`ok go then`); Expression Package E received (`approve Expression Package E`); Phase 2 spoken output received (`ok do that`, 2026-09-15); app 0.4.4 language-continuity hotfix received (`Approve the 0.4.4 language-continuity hotfix`, 2026-09-15); Phase 2c public-directory picker received (`yes update the doc and do it now please`, 2026-09-15); Phase 3 source hardening received (`you may have time to do Phase 3 already`, 2026-09-15); app 0.6.2 chat-deduplication hotfix and public community-page update received (`yes I would like you to do the 0.6.2 chat-deduplication hotfix...`, 2026-09-15)
...
Source steps 1–7 are complete in app 0.6.0. The reviewed manifest contains 39 files; native tests pass 65/65; the deterministic Phase 3 guardrail passes; and the official assistant passes a fresh temporary install, entry-point check, and uninstall. App 0.6.1 is now live through the existing GitHub-to-Hugging-Face sync with the browser-memory chat, last-answer audit, neutral starter, conditional HEA World default, matching-language voice-character preference, and truthful public-page copy. App 0.6.2 / `v0.6.2-rc.1` is the approved chat-only repair: one monotonic local turn id owns one pending assistant bubble, preventing the preceding completed state from being rendered into a newly submitted turn. It must match the expanded manifest exactly, synchronize through GitHub, and pass a catalog-installed one-turn/one-bubble owner check before being called released.
